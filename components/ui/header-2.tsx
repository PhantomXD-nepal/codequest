...
export function Header() {
	...
	return (
		<header
			className={cn(
				'sticky top-0 z-50 mx-auto w-full max-w-5xl border-b-4 border-black md:rounded-none md:border-4 md:transition-all md:ease-out mc-card',
				{
		...
		>
			...
			>
				<Link href="/" className="flex items-center gap-2 font-bold text-xl font-pixel mc-text-shadow text-white">
					CodeQuest
				</Link>
				<div className="hidden items-center gap-4 md:flex">
					{links.map((link, i) => (
						<a key={i} className={cn("font-pixel text-[10px] text-white hover:text-yellow-400 transition-colors mc-text-shadow")} href={link.href}>
							{link.label}
						</a>
					))}
					{session ? (
						<button onClick={handleSignOut} className="mc-button px-4 py-2 font-pixel text-[10px] text-white mc-text-shadow">Sign Out</button>
					) : (
						<>
							<Link href="/signin">
								<button className="mc-button px-4 py-2 font-pixel text-[10px] text-white mc-text-shadow">Sign In</button>
							</Link>
							<Link href="/signup">
								<button className="mc-button mc-button-green px-4 py-2 font-pixel text-[10px] text-white mc-text-shadow">Get Started</button>
							</Link>
						</>
					)}
				</div>
				<Button size="icon" variant="outline" onClick={() => setOpen(!open)} className="md:hidden border-2 border-black">
			...
			>
				...
				>
						...
						{links.map((link) => (
							<a
								...
								className={buttonVariants({
									variant: 'ghost',
									className: 'justify-start font-pixel text-xs',
								})}
								href={link.href}
							>
								{link.label}
					...
					<div className="flex flex-col gap-2">
						{session ? (
							<Button onClick={handleSignOut} className="w-full font-pixel text-xs">Sign Out</Button>
						) : (
							<>
								<Link href="/signin" className="w-full">
									<Button variant="outline" className="w-full font-pixel text-xs">
										Sign In
									</Button>
								</Link>
								<Link href="/signup" className="w-full">
									<Button className="w-full font-pixel text-xs">Get Started</Button>
								</Link>
							</>
						)}
					</div>
